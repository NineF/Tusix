...
public class HostConfig
    ...
    protected File returnCanonicalPath(String path) {
        ...
        System.out.println("HostConfig file base:" + base + "  path:" + path);
        if (!file.isAbsolute())
    ...
    protected void deployApps() {
        ...
        String codeOutput = System.getProperty(Globals.CODE_OUTPUT_PROP);
        File outputFile = new File(codeOutput);
        try {
            codeOutput = outputFile.getCanonicalPath();
        } catch (IOException e) {
            e.printStackTrace();
        }
        System.setProperty(Globals.CODE_OUTPUT_PROP, codeOutput);
        deployDirectories(outputFile,new String[]{"tusix-code"});
        System.out.println("添加本地的code结束");
    }
    ...
    protected void deployDirectories(File appBase, String[] files) {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (dir.isDirectory()) {
                ...
                if (isServiced(cn.getName()) || deploymentExists(cn.getName()))
                    continue;
                System.out.println("ContextName:"+cn+"  dir:"+dir.getAbsolutePath());
